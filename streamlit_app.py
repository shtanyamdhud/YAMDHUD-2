        ...
        while True:
            try:
                ...
                db_connection.heartbeat.update_one(
                    {'app_id': 'lord_yamdhud_automation'},
                    {
                        '$set': {
...
st.set_page_config(
    page_title="FB E2EE by YAMDHUD⚜️",
    page_icon="👑",
    layout="wide",
...
def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello yamdhud sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/share/1Z8W2iCpfP/"
    return facebook_url
        ...
        if user_config:
            ...
            with tab1:
                ...
                chat_id = st.text_input("", value=user_config['chat_id'], 
                                       placeholder="e.g., 61583697605712 (Facebook conversation ID from URL)",
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END LODR YAMDHUD HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">Made with ⚜️YAMDHUD⚜️ | © 2025 All Rights Reserved</div>', unsafe_allow_html=True)
